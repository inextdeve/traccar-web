...
const TypesManagement = () => {
  ...
  const setTypes = (data) => dispatch(dbManagementActions.setTypes(data));
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${URL}/api/types`, {
        ...
        if (response.status >= 400 && response.status < 600) {
          ...
          throw new Error(types.message || "Server Error: Cannot get types");
        }
  ...
  useEffect(() => {
    if (selected.length === 1 || openEdit) {
      ...
      const selectedTypes = types.filter((type) => type.id == id);

      setActiveRow(selectedTypes[0]);
    }
  }, [selected, openEdit]);
      ...
      case "edit":
        try {
          ...
          const newTypes = types.map((type) => {
            if (Number(type.id) === Number(activeRow.id)) {
              return activeRow;
            }
            return type;
          });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const newTypes = types.filter((type) => {
        return !selected.includes(type.id);
      });
  ...
  return (
    <PageLayout
      menu={<SettingsMenu />}
      breadcrumbs={["settingsTitle", "typesManagement"]}
    >
